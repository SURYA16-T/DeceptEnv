...
async def test_mitigator_race_condition() -> None:
    ...
    current_owner = mitigator.platform._get_current_user_sid() if os.name == 'nt' else os.getuid()
    with patch.object(mitigator.platform, "get_process_owner", return_value=current_owner):
...
def test_forensics_bounded_memory_read(tmp_path: Path) -> None:
    ...
    config = SecurityConfig(canary_directory=Path("/tmp"), max_forensic_dump_size_bytes=1048576) 
    forensics = ForensicsExtractor(config)
    ...
    with patch("os.name", "posix"):
        with patch("os.path.exists", return_value=True):
            ...
            def mock_open_file(path: str, mode: str = "r", *args: Any, **kwargs: Any) -> Any:
                if "maps" in str(path):
