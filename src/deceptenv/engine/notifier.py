...
class SlackNotifier:
    ...
    async def notify_incident(
        self, report: ForensicReport, pid: int, tripwire: Path
    ) -> None:
        """
        ...
        payload = {
            "blocks": [
                ...
                },
                {
                    "type": "section",
                    "fields": [
                        {"type": "mrkdwn", "text": f"*Offending PID:*\n{pid}"},
                        {"type": "mrkdwn", "text": f"*User UID:*\n{uid}"},
                    ],
                },
                {
                    "type": "section",
                    "fields": [
                        {"type": "mrkdwn", "text": f"*Binary Path:*\n`{binary_path}`"},
                        {"type": "mrkdwn", "text": f"*Tripwire:*\n`{tripwire}`"},
                    ],
                },
                {
                    "type": "section",
